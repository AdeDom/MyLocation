...
class MainActivity : BaseLocationActivity() {
    ...
    private val launcherForegroundService = registerForActivityResult(
        ActivityResultContracts.RequestMultiplePermissions()
    ) { result ->
        val isGranted = result.entries.all { it.value }
        if (isGranted) {
    ...
    private val launcherRequestOneTime = registerForActivityResult(
        ActivityResultContracts.RequestMultiplePermissions()
    ) { result ->
        val isGranted = result.entries.all { it.value }
        if (isGranted) {
            val intent = Intent(this, RequestOneTimeActivity::class.java)
            startActivity(intent)
        } else {
            startApplicationDetailSetting()
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnRequestOneTime.setOnClickListener {
            launcherRequestOneTime.launch(permissions)
        }
    }
}